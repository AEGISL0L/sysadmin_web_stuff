...
convert_to_webp() {
    ...
    for q in {65..80..5}; do
        for m in {4..6}; do
            for f in {40..90..10}; do
                local output_test="${WEBP_DIR}/${basename_file%.png}_q${q}_m${m}_f${f}.webp"
                cwebp -q "$q" -m "$m" -pass 20 -mt -sharp_yuv -af -pre 4 -f "$f" \
                      -alpha_filter best -alpha_method 1 -exact "$file" -o "$output_test" &>/dev/null

                size=$(stat -c%s "$output_test")
                if [ "$size" -lt "$best_size" ]; then
                    best_size=$size
                    best_params="q${q}_m${m}_f${f}"
                else
                    rm -f "$output_test"
                fi
            done
    ...
    # Mover el mejor archivo WebP al directorio final
    output="${WEBP_DIR}/${basename_file%.png}.webp"
    if [ -n "$best_params" ]; then
        mv -f "${WEBP_DIR}/${basename_file%.png}_${best_params}.webp" "$output" || {
            show_progress "Error al mover ${basename_file%.png}_${best_params}.webp a $output."
            exit 1
        }
    else
        # Si no se encontró una mejor compresión, crear una WebP básica
        cwebp -q 75 -mt -sharp_yuv -af -pre 4 -alpha_filter best -alpha_method 1 "$file" -o "$output" &>/dev/null
        best_size=$(stat -c%s "$output")
    fi
